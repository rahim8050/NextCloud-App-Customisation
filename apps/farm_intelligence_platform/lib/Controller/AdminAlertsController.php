...
final class AdminAlertsController extends Controller {
	...
	public function broadcast(): JSONResponse {
		...
		$this->logEndpointEntry('broadcast alert', $requestId);
		$operation = [];
		$path = '';

		try {
			$operation = $this->schemaService->getOperation(self::OPERATION_IDS['broadcast'], $requestId);
			$path = (string)($operation['path'] ?? '');
			$params = $this->stripPathParams($this->request->getParams(), $path);
			$body = $this->filterBodyParams($params, $operation['bodyFields'] ?? [], false);
			$this->logProxyRequest('broadcast alert', $operation, $path, [], $requestId);
			...
			$this->logProxyResponse('broadcast alert', $operation, $path, $requestId, $result['statusCode']);
			$payload = $result['payload'];
		} catch (WeatherApiException $e) {
			return $this->handleWeatherApiException($e, $requestId, 'broadcast alert');
		} catch (\Throwable $t) {
			return $this->handleUnexpectedError($t, $requestId, 'broadcast alert');
		}
		return $this->buildSuccessResponse($payload, 'Alert broadcast sent.', Http::STATUS_CREATED);
	}
	...
	private function logEndpointEntry(string $action, string $requestId): void {
		...
		$this->logger->debug(
			'Weather API admin alerts endpoint hit',
	...
	private function logProxyRequest(
		...
		$this->logger->debug(
			'Weather API admin alerts proxy request',
			LogSanitizer::sanitizeContext([
				'action' => $action,
				'requestId' => $requestId,
	...
	private function logProxyResponse(
		...
		$this->logger->debug(
			'Weather API admin alerts proxy response',
			LogSanitizer::sanitizeContext([
				'action' => $action,
				'requestId' => $requestId,
				'method' => (string)($operation['method'] ?? ''),
	...
	private function handleWeatherApiException(WeatherApiException $exception, string $requestId, string $action): JSONResponse {
		$this->logger->warning(
			'Weather API admin alerts request failed',
			LogSanitizer::sanitizeContext([
				'action' => $action,
				'requestId' => $requestId,
