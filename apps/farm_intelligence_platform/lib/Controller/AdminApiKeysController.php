...
final class AdminApiKeysController extends Controller {
	...
	public function listKeys(): JSONResponse {
		...
		$this->logEndpointEntry('list api keys', $requestId);
		$operation = [];
		$path = '';
		...
		try {
			...
			$this->logProxyRequest('list api keys', $operation, $path, [], $requestId);
			...
			$this->logProxyResponse('list api keys', $operation, $path, $requestId, $result['statusCode']);
			$payload = $result['payload'];
		} catch (WeatherApiException $e) {
			return $this->handleWeatherApiException($e, $requestId, 'list api keys');
		} catch (\Throwable $t) {
			return $this->handleUnexpectedError($t, $requestId, 'list api keys');
		}
		return $this->buildSuccessResponse($payload, 'API keys loaded.');
	}

	#[AdminRequired]
	public function createKey(): JSONResponse {
		$requestId = $this->resolveRequestId();
		$this->logEndpointEntry('create api key', $requestId);
		$operation = [];
		$path = '';

		try {
			$operation = $this->schemaService->getOperation(self::OPERATION_IDS['create'], $requestId);
			$path = (string)($operation['path'] ?? '');
			$params = $this->stripPathParams($this->request->getParams(), $path);
			$body = $this->filterBodyParams($params, $operation['bodyFields'] ?? [], false);
			$this->logProxyRequest('create api key', $operation, $path, [], $requestId);
			$result = $this->weatherApiClient->requestJsonWithStatus(
			...
			$this->logProxyResponse('create api key', $operation, $path, $requestId, $result['statusCode']);
			$payload = $result['payload'];
		} catch (WeatherApiException $e) {
			return $this->handleWeatherApiException($e, $requestId, 'create api key');
		} catch (\Throwable $t) {
			return $this->handleUnexpectedError($t, $requestId, 'create api key');
		}
		$response = $this->buildSuccessResponse($payload, 'API key created.', Http::STATUS_CREATED);
		$response->addHeader('Cache-Control', 'no-store');
		return $response;
	}

	#[AdminRequired]
	public function revokeKey(string $pk): JSONResponse {
		$requestId = $this->resolveRequestId();
		$this->logEndpointEntry('revoke api key', $requestId);
		$operation = [];
		$path = '';

		try {
			$operation = $this->schemaService->getOperation(self::OPERATION_IDS['revoke'], $requestId);
			$pathTemplate = (string)($operation['path'] ?? '');
			$path = $this->applyPathParams($pathTemplate, ['pk' => $pk]);
			$this->logProxyRequest('revoke api key', $operation, $path, [], $requestId);
			$result = $this->weatherApiClient->requestJsonWithStatus(
			...
			$this->logProxyResponse('revoke api key', $operation, $path, $requestId, $result['statusCode']);
			$payload = $result['payload'];
		} catch (WeatherApiException $e) {
			return $this->handleWeatherApiException($e, $requestId, 'revoke api key');
		} catch (\Throwable $t) {
			return $this->handleUnexpectedError($t, $requestId, 'revoke api key');
		}
		return $this->buildSuccessResponse($payload, 'API key revoked.');
	}

	#[AdminRequired]
	public function rotateKey(string $pk): JSONResponse {
		$requestId = $this->resolveRequestId();
		$this->logEndpointEntry('rotate api key', $requestId);
		$operation = [];
		$path = '';

		try {
			$operation = $this->schemaService->getOperation(self::OPERATION_IDS['rotate'], $requestId);
			$pathTemplate = (string)($operation['path'] ?? '');
			$path = $this->applyPathParams($pathTemplate, ['pk' => $pk]);
			$params = $this->stripPathParams($this->request->getParams(), $pathTemplate);
			$body = $this->filterBodyParams($params, $operation['bodyFields'] ?? [], false);
			$this->logProxyRequest('rotate api key', $operation, $path, [], $requestId);
			$result = $this->weatherApiClient->requestJsonWithStatus(
				(string)($operation['method'] ?? 'POST'),
				$path,
				[],
			...
			$this->logProxyResponse('rotate api key', $operation, $path, $requestId, $result['statusCode']);
			$payload = $result['payload'];
		} catch (WeatherApiException $e) {
			return $this->handleWeatherApiException($e, $requestId, 'rotate api key');
		} catch (\Throwable $t) {
			return $this->handleUnexpectedError($t, $requestId, 'rotate api key');
		}
		$response = $this->buildSuccessResponse($payload, 'API key rotated.', Http::STATUS_CREATED);
		$response->addHeader('Cache-Control', 'no-store');
		return $response;
	}
	...
	private function logEndpointEntry(string $action, string $requestId): void {
		...
		$this->logger->debug(
			'Weather API admin keys endpoint hit',
	...
	private function logProxyRequest(
		...
		$this->logger->debug(
			'Weather API admin keys proxy request',
			LogSanitizer::sanitizeContext([
				'action' => $action,
				'requestId' => $requestId,
	...
	private function logProxyResponse(
		...
		$this->logger->debug(
			'Weather API admin keys proxy response',
			LogSanitizer::sanitizeContext([
				'action' => $action,
				'requestId' => $requestId,
				'method' => (string)($operation['method'] ?? ''),
	...
	private function handleWeatherApiException(WeatherApiException $exception, string $requestId, string $action): JSONResponse {
		$this->logger->warning(
			'Weather API admin keys request failed',
			LogSanitizer::sanitizeContext([
				'action' => $action,
				'requestId' => $requestId,
