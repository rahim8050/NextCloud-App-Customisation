...
```bash
curl -k -sS https://127.0.0.1/nextcloud/index.php/apps/farm_intelligence_platform/api/v1/admin/farms/schema
```
...
```bash
curl -k -sS https://127.0.0.1/nextcloud/index.php/apps/farm_intelligence_platform/api/v1/admin/farms/list
```
...
curl -k -sS -X POST \
  -H "Content-Type: application/json" \
  -d '{"name":"Codex Farm A"}' \
  https://127.0.0.1/nextcloud/index.php/apps/farm_intelligence_platform/api/v1/admin/farms/create
```
...
curl -k -sS -X PATCH \
  ...
  https://127.0.0.1/nextcloud/index.php/apps/farm_intelligence_platform/api/v1/admin/farms/<ID>
```
...
```
{"reqId":"FG56lDJCpAy0cvYzCtw6","app":"farm_intelligence_platform","method":"GET","url":"/nextcloud/index.php/apps/farm_intelligence_platform/api/v1/admin/farms/schema","message":"Current user is not logged in"}
{"reqId":"UVkhciUWYJR9Jq6LcZJf","app":"farm_intelligence_platform","method":"GET","url":"/nextcloud/index.php/apps/farm_intelligence_platform/api/v1/admin/farms/list","message":"Current user is not logged in"}
```

## E) UI proof steps (blocked pending auth)

1) Open Nextcloud admin settings → Farm Intelligence Platform → Farms
...
- `occ router:list` fails due to config permissions (needs to run as `www-data` owner):
  ...
  - Fix: run `sudo -u www-data php /var/www/html/nextcloud/occ router:list | rg -i farm_intelligence_platform | rg -i farms`
