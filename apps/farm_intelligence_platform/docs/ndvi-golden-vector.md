...
curl -sS "${NC_BASE}/index.php/apps/farm_intelligence_platform/api/v1/admin/farms/${FARM_ID}/ndvi/latest?lookback_days=14&max_cloud=30" \
...
curl -sS "${NC_BASE}/index.php/apps/farm_intelligence_platform/api/v1/admin/farms/${FARM_ID}/ndvi/timeseries?start=2024-01-01&end=2024-02-01&max_cloud=30" \
  -H "OCS-APIRequest: true" \
  -H "requesttoken: $NC_REQUESTTOKEN" \
  -b "$NC_COOKIE"
```
...
curl -sS "${NC_BASE}/index.php/apps/farm_intelligence_platform/api/v1/admin/farms/${FARM_ID}/ndvi/raster/queue" \
  -H "OCS-APIRequest: true" \
  -H "requesttoken: $NC_REQUESTTOKEN" \
  -H "Content-Type: application/json" \
  -b "$NC_COOKIE" \
...
curl -sS "${NC_BASE}/index.php/apps/farm_intelligence_platform/api/v1/admin/farms/${FARM_ID}/ndvi/raster.png?date=2024-02-01" \
  -H "OCS-APIRequest: true" \
  -H "requesttoken: $NC_REQUESTTOKEN" \
  -b "$NC_COOKIE" \
  -o /tmp/ndvi.png
...
In **Settings → Farm Intelligence Platform → Farms → NDVI**:
